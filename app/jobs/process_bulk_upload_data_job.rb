class ProcessBulkUploadDataJob < ApplicationJob
  ...
  def perform(bulk_upload_id, index, line)
    ...
    case type
    when "U"
      create_user(bulk_upload, data, (index + 1))
    when "E"
      create_company(bulk_upload, data, (index + 1))
    when "V"
      create_job(bulk_upload, data, (index + 1))
    else
      add_error_log(bulk_upload, (index + 1), "Linha inválida")
    end
    ...
    Turbo::StreamsChannel.broadcast_update_to(
      ...
      locals: {
        ...
        errors_details: JSON.parse(redis.get("bulk-upload-#{bulk_upload.id}-errors-details") || "[]").sort_by! { |msg| msg.match(/Linha (\d+)/)&.captures&.first.to_i },
  ...
  def create_user(bulk_upload, data, line_number)
    ...
    if user.valid?
      redis.incr("bulk-upload-#{bulk_upload.id}-successful") if user.save
    else
      user.errors.full_messages.each do |error|
        add_error_log(bulk_upload, line_number, error)
      end
    redis.incr("bulk-upload-#{bulk_upload.id}-errors-count")
    end
  end


  def create_company(bulk_upload, data, line_number)
    ...
    if company.valid?
      redis.incr("bulk-upload-#{bulk_upload.id}-successful") if company.save
    else
  ...
  def create_job(bulk_upload, data, line_number)
    ...
    job = JobPosting.new(title: title, description: description, salary: salary, salary_currency: salary_currency.downcase, salary_period: formated_salary_period, work_arrangement: formated_work_arrangement, job_type_id: job_type_id, job_location: job_location, experience_level_id: experience_level_id, company_profile_id: company_id)

    if job.valid?
      redis.incr("bulk-upload-#{bulk_upload.id}-successful") if job.save
    else
  ...
  def add_error_log(bulk_upload, line_number, error)
    errors_details = JSON.parse(redis.get("bulk-upload-#{bulk_upload.id}-errors-details") || "[]")
    errors_details.push("Linha #{line_number}: #{error}")
    redis.set("bulk-upload-#{bulk_upload.id}-errors-details", errors_details.to_json)

    BulkUploadError.create!(bulk_upload: bulk_upload, line: line_number, message: error)
  end
